...
2. **Two federal datasets are fully public and go back decades.** BLM's Oregon/Washington prescribed fire polygons (36,966 regional treatment records (36,965 completed, one active), with a `REASON` field and a NEPA plan ID) and the Forest Service FACTS hazardous fuel treatments (20,531 Oregon fire-treatment records, with NEPA document names and a purpose code). Coverage is incomplete: 364 FACTS records describe treatments consumed by wildfire and 16,476 describe pile burning. BLM has 33,558 missing or common-placeholder PLANID values, so most document links cannot be automated. They can be pulled today.
...
5. **Important records still need acquisition:** DEQ habitat/wetland restoration permits, LRAPA ecological permits and accomplishments, refuge/park histories, additional treatment documentation, the ODA Willamette Valley field burning program (grass seed residue, up to 15,000 acres a year, registered by April 1), and anything historical from ODF's SDS. Backyard and debris burning permits are issued by local fire districts and are not centralized anywhere; treat that as out of scope for a first version.

## Tier 1: prescribed fire records

### ODF Smoke Management Program (forest-management smoke registry)

ODF’s smoke-management scope excludes agricultural burning, construction and land-use changes. A smoke registration is not a complete burn-permit inventory. Request registrations, authorizations, individual ignitions and accomplishments separately, including join keys. ODF meteorologists issue daily burn instructions. The data is entered by ODF field offices and by federal units into the Smoke Data System.
...
| Underlying feature service (four layers) | https://gis.odf.oregon.gov/odfags/rest/services/Fire/Fire_Smoke_Management/FeatureServer | Public, queried. Point geometry. Returned zero rows on September 10. This establishes only that the endpoint returned no records; the reason is unknown. |
...
Some records may overlap ODF reporting, but coverage must be confirmed. Begin partnership conversations early and agree on attribution, location precision and review before publishing identified tribal/cultural prescribed-burn records. Sources:
...
- **Prescribed burn associations** (10 in the September 11 OSU directory): Rogue Valley, Umpqua, Mt. Adams, Central Oregon Prescribed Burn Co-op, North Valley, Northeast Oregon, Southern Blues, Mid Valley, South Willamette, South Coast Cultural Burn Association. OSU Extension keeps the list: https://extension.oregonstate.edu/fire-program/prescribed-burn-associations (directory verified September 11; automated access may vary).
...
| FPA FOD v7 | 1992–2024, national | Historical occurrence baseline. Use edition-seven SQLite/GeoPackage download; catalog map link still serves v6. | https://www.fs.usda.gov/rds/archive/catalog/RDS-2013-0009.7 | Downloaded; checksum verified September 11 |
...
- **Treatment effectiveness.** FTEM (fuel treatment effectiveness monitoring) records every time a wildfire meets a prior treatment. It lives in IFTDSS behind a NIFC login; the public dashboard is at https://nifc.maps.arcgis.com/apps/dashboards/98de26c0239649858df20c1c9536676d. Worth a request once we have the treatment layers, for documented assessments; spatial overlap alone does not establish treatment effectiveness.
...
| Live per-burn feed | Archive all four ODF layers and the PNW tracker every six hours, preserving versions and actual last-success times. |
...
- Archive rolling feeds every six hours. Publisher retention is unconfirmed. Absence from a later feed does not prove cancellation or completion; a valid empty response differs from a failed request.
...
- ODF fire occurrence is in EPSG:2992 (Oregon Lambert, feet). BLM and FACTS declare NAD83. Explicitly request outSR=4326 from ArcGIS, preserve source spatial-reference metadata, and label geometry accuracy. Federal polygons are not inherently surveyed units.
- Use a full object-ID inventory and bounded requests, splitting truncated responses; validate final inventories before atomic publication. Do not stop at a service record limit.

## September 11 additions and source hierarchy

Use FPA FOD v7 for historical occurrences, Oregon Explorer for historical perimeters, and recent WFIGS for provisional additions. ODF occurrence and NWCC are reconciliation/enrichment sources; do not add their counts to the baseline. Verified IRWIN IDs link records without erasing their separate geometry meanings. Uncertain matches stay separate.

- **DEQ ecological permits:** habitat/wetland restoration applications may hold the site-selection justification, diagrams, recommendations and permit conditions. https://www.oregon.gov/deq/aq/pages/burning-101.aspx
- **LRAPA / Rivers to Ridges:** published permitted-unit map; request its GIS, annual permits, schedules and actual burn dates/acres. A permitted unit is not a confirmed burn. https://www.lrapa-or.gov/air-quality-protection/community-center/prescribed-burns/
- **USFWS and NPS:** refuge/prairie/wetland and park unit histories, plans, objectives and monitoring. Late summer/early fall burning disproves any assumption that September is necessarily empty. https://www.fws.gov/refuge/william-l-finley/what-we-do and https://www.nps.gov/crla/learn/nature/fire-management.htm
- **USGS Land Treatment Digital Library:** use the public release first; ask Justin Welty about Oregon coverage, documents and BLM overlap. https://www.usgs.gov/data/usgs-land-treatment-digital-library-data-release-a-centralized-archive-land-treatment-tabular
- **Smoke:** EPA AQS/AirData historical measurements, AirNow current observations, NOAA HMS polygons, and ODF/DEQ intrusion investigations. Measurements alone do not identify the responsible burn. https://www.epa.gov/aqs/obtaining-aqs-data and https://www.ospo.noaa.gov/products/land/hms.html
- **Effects:** MTBS size thresholds exclude many small Oregon burns. RAVG, FTEM and treatment monitoring remain separate acquisition workstreams. https://www.mtbs.gov/faqs
- **Documented story:** OSU’s Woodpecker account explains selection of oak/madrone and ponderosa pine units and patchy outcomes. No map pin until geometry is verified. https://www.forestry.oregonstate.edu/news/fire-purpose

All original context leads are retained above. Unexercised links, legal/policy summaries and access assumptions need verification before use in public captions. No steward has confirmed completeness.
