    ...
    <div className="space-y-12">
      ...
      <div>
        ...
        <ul className="mt-4 divide-y divide-white/10 overflow-hidden rounded-sm border border-white/15 bg-white/[0.04]">
          {DECIDE_NOW.map((d) => (
            <li key={d.decision} className="grid gap-x-6 gap-y-2 p-4 sm:grid-cols-[112px_minmax(0,1fr)] sm:p-5">
              <span
                className={`inline-flex h-7 w-fit items-center rounded-sm px-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.14em] ${
                  d.verdict === "now"
              ...
              <div>
                ...
                <p className="mt-1 text-[13px] leading-relaxed text-white/65">{d.why}</p>
              </div>
            </li>
          ))}
        </ul>
        <p className="mt-3 max-w-3xl text-[13.5px] leading-relaxed text-white/70">
        ...
        <ol className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {READINESS_DELIVERABLES.map((d) => (
            <li key={d.n} className="flex flex-col rounded-sm border border-white/15 bg-white/[0.04] p-5">
              ...
              <p className="mt-2 flex-1 text-[13px] leading-relaxed text-white/70">{d.body}</p>
              <p className="mt-4 border-t border-white/10 pt-3 text-[12px] leading-snug text-[var(--color-ember-bright)]">
                <span className="font-mono text-[9.5px] font-bold uppercase tracking-[0.14em] text-white/45">Gate · </span>
                {d.gate}
              </p>
            </li>
          ))}
            ...
            {NINETY_DAYS.map((s) => (
              <li key={s.day} className="relative">
                <div className="flex items-center gap-3">
                  <span className="relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--color-ember)] font-mono text-[12px] font-bold text-[var(--color-canopy)]">
                    {s.day}
                  </span>
                  <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/50">days</span>
                </div>
                <p className="mt-3 text-[13.5px] leading-relaxed text-white/85">{s.result}</p>
                <p className="mt-2.5 border-l-2 border-[var(--color-clay)] pl-3 text-[12px] leading-snug text-white/60">
                  <span className="font-mono text-[9.5px] font-bold uppercase tracking-[0.14em] text-[var(--color-ember)]">Stop rule · </span>
                  {s.stop}
                </p>
              </li>
            ))}
        ...
        <ol className="mt-4 grid gap-x-8 gap-y-3 sm:grid-cols-2">
          {CONDITIONS.map((c, i) => (
            <li key={c.title} className="flex gap-3">
              <span className="shrink-0 font-mono text-[12px] font-bold tabular-nums text-[var(--color-ember-bright)]">
                {String(i + 1).padStart(2, "0")}
              </span>
              <div>
                <p className="text-[14px] font-semibold text-white">{c.title}</p>
                <p className="mt-0.5 text-[12.5px] leading-snug text-white/60">{c.body}</p>
              </div>
            </li>
          ))}
        </ol>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {OPTIONS_AFTER.map((o) => (
            <div key={o.n} className={`rounded-sm p-5 ${o.n === 3 ? "border border-[var(--color-ember)]/60 bg-[var(--color-ember)]/10" : "border border-white/15 bg-white/[0.04]"}`}>
              <p className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-white/45">Option {o.n}</p>
              <h3 className="mt-1 font-editorial text-[20px] leading-snug text-white">{o.title}</h3>
              <p className="mt-2 text-[13px] leading-relaxed text-white/70">{o.body}</p>
              <p className="mt-3 border-t border-white/10 pt-2.5 font-mono text-[11px] leading-snug text-[var(--color-ember-bright)]">{o.money}</p>
            </div>
          ))}
        </div>
      </div>
