 ...
 *
 * Usage:
 *   npx tsx scripts/sync-crime.ts                # incremental (current + prev year)
 *   npx tsx scripts/sync-crime.ts --full          # re-sync all years 2015-2026
 *   npx tsx scripts/sync-crime.ts --year 2024     # sync a specific year only
 */
...
if (!DB_URL) {
  ...
  console.error(
    "Run with: set -a && source .env.local && set +a && npx tsx scripts/sync-crime.ts"
  );
  process.exit(1);
}
