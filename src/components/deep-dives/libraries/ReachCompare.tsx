  ...
  return (
    <div className="space-y-6">
      <div className="rounded-sm border border-[var(--color-parchment)] bg-white p-5 sm:p-7">
            ...
            return (
              <li key={r.id} className="grid grid-cols-[minmax(0,150px)_minmax(0,1fr)_56px] items-center gap-3 sm:grid-cols-[minmax(0,210px)_minmax(0,1fr)_64px]">
                <div className="min-w-0">
                  <p className={`truncate text-[13.5px] font-semibold ${isPortland ? "text-[var(--color-ink)]" : "text-[var(--color-ink-light)]"}`}>{r.label}</p>
                  <p className="truncate text-[11px] text-[var(--color-ink-muted)]">{r.measure}</p>
                </div>
        ...
        <p className="mt-4 text-[11.5px] leading-relaxed text-[var(--color-ink-muted)]">
      ...
      <div className="grid gap-px overflow-hidden rounded-sm border border-[var(--color-parchment)] bg-[var(--color-parchment)] sm:grid-cols-2 xl:grid-cols-4">
        {BENCHMARKS.map((b) => (
          <article key={b.city} className="bg-white p-4 sm:p-5">
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--color-ember)]">{b.city}</p>
            <h3 className="mt-0.5 font-editorial text-[18px] leading-tight text-[var(--color-ink)]">{b.system}</h3>
            <p className="mt-1.5 font-mono text-[11px] tabular-nums leading-snug text-[var(--color-canopy)]">{b.stat}</p>
            <p className="mt-2 text-[12.5px] leading-snug text-[var(--color-ink-light)]">{b.lesson}</p>
          </article>
        ))}
      </div>
    </div>
