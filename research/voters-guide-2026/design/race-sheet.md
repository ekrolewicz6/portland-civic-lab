...
## Revised September 20 (evening): names on the first screen

After the owner reviewed the phone page ("way too much text going on here"), the race page was rebuilt around one rule: on a phone, the first candidate's card is visible on the first screen, and nothing above it is longer than a line.

- **Masthead.** The district name, four facts as numerals (candidates, seats, the sourced ranking limit, the return deadline), and two lines of neighborhoods with the other district's link. The ranking limit is read from the sourced ballot instruction, never from the seat count; when no instruction is sourced the fact shows the race's method instead.
- **The choice in one paragraph** keeps its place and its "Our reading" label, with the draft state as a tag rather than a sentence. On phones it folds to two lines with a More control; from 769px up it shows in full beside the title.
- **Rail.** Five chips (All · Homes · Safety · Money · Streets) that stick under the site header on phones; the (i) buttons are gone and the issue's question appears beneath the rail when a chip is pressed. A pressed chip now shows one column: each card keeps only that tile, full width, so the whole field on one issue reads as a list of names and stances. Desktop keeps its column headers and dims the other columns.
- **Cards.** On phones each cell is a tinted tile with its label inside (Homes · Safety · Money · Streets); gaps are dashed tiles that say "Not found"; the Save control is an icon in the card's corner. The table, its semantics, the open detail and the ladder are unchanged.
- **Compare on more** is one toolbar control that opens the eight topic chips; selected topics show as a count on the control. Nothing else sits between the rail and the first card.
- **About this race** (the foot of the page) now carries what the masthead used to: how to vote with its source, what this Council can do, and how the list was made, then the standards links and the glossary.

Nothing in the research objects, the overlay lines or the routes changed; the e2e suite was updated for the short rail labels, the single-column phone view and the picker disclosure.
