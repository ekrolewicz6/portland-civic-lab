    ...
    <div className="space-y-6">
      <div className="overflow-hidden rounded-sm border border-[var(--color-parchment)] bg-white">
        <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-2 border-b border-[var(--color-parchment)] bg-[var(--color-paper-warm)] px-5 py-4 sm:px-7">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--color-ink-muted)]">
          ...
          <svg viewBox={`0 0 ${VW} ${VH}`} className="h-auto w-full min-w-[640px]" role="img" aria-label="Line chart: physical circulation fell from 24 million in 2011 to 10 million in 2025; digital circulation rose from 3.5 million in 2019 to 7.4 million in 2025; in-person visits fell from 5.5 million to 2.2 million.">
            ...
            {[0, 5, 10, 15, 20, 25].map((m) => (
              <g key={m}>
                ...
                <text x={PAD.l - 8} y={y(m) + 3.5} textAnchor="end" fontSize="10" fill="#78716c" fontFamily="var(--font-mono)">
                  {m}M
                </text>
              </g>
            ))}
            {/* x labels */}
            {YEARS.map((p) => (
              <text key={p.fy} x={x(p.fy)} y={VH - PAD.b + 18} textAnchor="middle" fontSize="10.5" fill="#44403c" fontFamily="var(--font-mono)">
                FY{p.fy}
              </text>
            ))}
            ...
            <text x={x(2020.85)} y={VH - PAD.b - 8} textAnchor="middle" fontSize="9" fill="#b85c3a" fontFamily="var(--font-mono)" letterSpacing="0.5">
              CLOSED
            </text>
            <rect x={x(2021.5)} y={PAD.t} width={x(2026) - x(2021.5)} height={VH - PAD.t - PAD.b} fill="#c8956c" fillOpacity="0.06" />
            <text x={x(2026) - 4} y={PAD.t + 12} textAnchor="end" fontSize="9" fill="#a06f45" fontFamily="var(--font-mono)" letterSpacing="0.5">
            ...
            <text x={x(2013) + 5} y={VH - PAD.b - 8} fontSize="9" fill="#0f2419" fillOpacity="0.7" fontFamily="var(--font-mono)" letterSpacing="0.5">
            ...
            {/* endpoint labels */}
            <text x={x(2011) - 6} y={y(23.95) - 8} fontSize="11" fontWeight="700" fill={SERIES.physical.color} fontFamily="var(--font-mono)" textAnchor="start">24.0M</text>
            <text x={x(2025) + 8} y={y(10.15) + 4} fontSize="11" fontWeight="700" fill={SERIES.physical.color} fontFamily="var(--font-mono)">10.2M</text>
            <text x={x(2025) + 8} y={y(7.37) + 4} fontSize="11" fontWeight="700" fill={SERIES.digital.color} fontFamily="var(--font-mono)">7.4M</text>
            <text x={x(2025) + 8} y={y(2.18) + 4} fontSize="11" fontWeight="700" fill={SERIES.visits.color} fontFamily="var(--font-mono)">2.2M</text>
            <text x={x(2011) - 6} y={y(5.52) - 8} fontSize="11" fontWeight="700" fill={SERIES.visits.color} fontFamily="var(--font-mono)">5.5M</text>
            <text x={x(2019)} y={y(3.46) + 16} fontSize="10" fontWeight="700" fill={SERIES.digital.color} fontFamily="var(--font-mono)" textAnchor="middle">3.5M</text>
          </svg>
        </div>
        <p className="border-t border-[var(--color-parchment)] px-5 py-3 text-[11.5px] leading-relaxed text-[var(--color-ink-muted)] sm:px-7">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div key={t.k} className={`rounded-sm border-l-[3px] bg-white p-4 ${t.tone === "clay" ? "border-l-[var(--color-clay)]" : "border-l-[var(--color-ember)]"}`}>
            ...
            <p className="mt-1.5 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--color-ink-muted)]">{t.k}</p>
            <p className="mt-2 text-[12.5px] leading-snug text-[var(--color-ink-light)]">{t.note}</p>
          </div>
        ))}
      </div>
    </div>
