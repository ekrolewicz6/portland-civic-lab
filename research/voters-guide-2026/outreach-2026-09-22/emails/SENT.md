# What was sent (September 22, 2026)

The follow-up emails in this folder went out from edan@portlandciviclab.org on the evening of September 22, 2026, between 5:37 and 7:28 PM PDT, one at a time, about a minute apart. 114 were sent and none failed. The per-message ledger (recipient, Gmail message and thread ids, send time) is private and stays in `runtime-data/voters-guide-2026/outreach/2026-09-22-final-results.jsonl`.

## How they were sent

- 85 drafts were rebuilt before sending from the data as it stood that evening (the Sollitt and Green changes had landed since the drafts were made), 26 went out as drafted, and 3 Council replies (Kimberly Tucker, Martin Ward, Mitch Green) were sent as fresh replies in their September 19 threads, because editing a reply draft pulls it out of its thread.
- The eight scheduled send tasks set up first did not send anything: a scheduled session cannot answer a permission prompt, so each one stalled at its first send. The sends ran from an attended session instead, paced about a minute apart.
- The Gmail connector rewrites any `https://` or lowercase-domain link into a `google.com/url` redirect when it writes a draft or sends. Every link in these emails is written as `PortlandCivicLab.org/voters-guide/...`, which survives and still auto-links, and a test send was checked raw before the batch.

## Not emailed

- Held on purpose: Tom Sollitt (he had just answered every gap and wrote that more is coming), Steve Novick (his follow-up went September 21), Jayne Cronlund (she said on September 20 that answers are coming this week) and Ali Beaudoin (Edan sent her the current list by hand at 3:38 PM after her note that answers are coming).
- Contact form only: 30 campaigns publish a form and no email. Their drafts are in the race files here, ready to paste; nothing was submitted.
- No public channel: 5 candidates (Tim E Nelson, Melisa Finkle, John A N Chee, Cristian Salgado, Luis Garcia).

## Delivery problems

- Betty Mumm (Oregon City Commission): the address on file bounced (550, address not found). Her page needs another channel.

## Replies by 7:30 PM PDT

- Keir Legree answered all seven questions; folded into his page the same night (research log `legree-2026-09-22`).
- Kellie Torres, John Goldsmith and Matt Schulte wrote that full answers are coming in the next day.
- Sharon Meieran’s campaign (Corin) and Tiffany Koyama Lane (connecting us with Lydia on her team) acknowledged and said answers are coming.

## Replies by 1 AM PDT, September 23

- Cristal Otero answered all eight follow-up questions, TJ Anderson answered five of the Council’s choices in a word or two, and Tom Sollitt answered his three September 19 questions. All three were folded in (research log `otero-2026-09-22`, `anderson-2026-09-22`, `sollitt-2026-09-23`).
- John McDonald asked that “former” come off “journalist”; corrected (`mcdonald-2026-09-22`). His answers are coming.
- Barbara J Kahl (U.S. House, OR-1) will revise all twelve answers, objected to paraphrase and asked the deadline. Reply sent: no hard deadline; anything by October 7 is on the page before ballots mail October 14; answers appear in her own words on her brief.
- Julia Brim-Edwards will answer next week (reply sent with the October 7 timing). The James Rhodes campaign will review its page. Tiffany Koyama Lane wrote that Lydia on her team would follow up, but Lydia was not copied; reply sent asking for her address.
- Matt Schulte asked to send complete answers the next day; reply sent saying yes.
