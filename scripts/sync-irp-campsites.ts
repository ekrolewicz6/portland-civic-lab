 ...
 *
 * Usage:
 *   npx tsx scripts/sync-irp-campsites.ts           # incremental (default)
 *   npx tsx scripts/sync-irp-campsites.ts --full     # full re-sync
 */
...
if (!DB_URL) {
  ...
  console.error(
    "Run with: set -a && source .env.local && set +a && npx tsx scripts/sync-irp-campsites.ts"
  );
  process.exit(1);
}
