        ...
        <div className="grid gap-px bg-[var(--color-parchment)] md:grid-cols-4">
          {[
            ...
            <div key={n.who} className="relative bg-white p-5">
              <span className={`inline-flex items-center rounded-sm px-2 py-1 font-mono text-[10px] font-bold uppercase leading-tight tracking-[0.12em] ${WHO_TONE[n.tone]}`}>
                {n.sets}
              </span>
              <h3 className="mt-2.5 font-editorial text-[18px] leading-snug text-[var(--color-ink)]">{n.who}</h3>
              <p className="mt-1.5 text-[12.5px] leading-snug text-[var(--color-ink-light)]">{n.body}</p>
              {i < 3 ? (
                <span className="absolute -right-3 top-1/2 z-10 hidden h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-[var(--color-parchment)] bg-white font-mono text-[12px] text-[var(--color-ink-muted)] md:flex">
                  →
                </span>
              ) : null}
            </div>
        ...
        {[
          ...
          <div key={col.title} className="overflow-hidden rounded-sm border border-[var(--color-parchment)] bg-white">
            <div className="border-b border-[var(--color-parchment)] bg-[var(--color-paper-warm)] px-5 py-3">
              <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--color-ember)]">{col.title}</p>
            </div>
            <ul className="divide-y divide-[var(--color-parchment)]">
              {col.rows.map((a) => (
                <li key={a.action} className="grid grid-cols-[92px_minmax(0,1fr)] gap-3 px-5 py-3.5 sm:grid-cols-[108px_minmax(0,1fr)]">
                  <span className={`inline-flex h-6 w-fit items-center whitespace-nowrap rounded-sm px-2 font-mono text-[10px] font-bold uppercase tracking-[0.1em] ${WHO_TONE[a.who]}`}>
                    {WHO_SHORT[a.who]}
                  </span>
                  <div>
                    <p className="text-[13.5px] font-semibold leading-snug text-[var(--color-ink)]">
                      {a.action}
                      {a.status ? <span className="ml-2 font-mono text-[10px] font-normal uppercase tracking-[0.1em] text-[var(--color-clay)]">{a.status}</span> : null}
                    </p>
                    <p className="mt-0.5 text-[12px] leading-snug text-[var(--color-ink-muted)]">{a.how}</p>
                  </div>
                </li>
              ))}
            </ul>
