    ...
    <div className="grid gap-4 lg:grid-cols-5">
      ...
      <div className="rounded-sm border border-[var(--color-parchment)] bg-white p-5 sm:p-7 lg:col-span-3">
        ...
        <div className="relative mt-12 h-12">
          ...
          />
          <span className="absolute left-3 top-1/2 -translate-y-1/2 font-mono text-[11px] font-semibold uppercase tracking-[0.12em] text-white">
            Levied · $1.22
          </span>
          <span
            className="absolute top-full mt-2 -translate-x-full whitespace-nowrap font-mono text-[11px] font-bold text-[var(--color-clay)]"
            style={{ left: "100%" }}
          >
            {DERIVED.levyHeadroomPct}% headroom · $0.02
          </span>
          <span className="absolute -top-2 h-[calc(100%+16px)] w-px bg-[var(--color-ink)]" style={{ left: "100%" }} />
          <span className="absolute bottom-full mb-1.5 -translate-x-full whitespace-nowrap font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--color-ink)]" style={{ left: "100%" }}>
            Legal cap, set by voters 2012
          </span>
        </div>

        <p className="mt-10 max-w-xl text-[13.5px] leading-relaxed text-[var(--color-ink-light)]">
        ...
        <div className="rounded-sm border border-[var(--color-parchment)] bg-white p-5">
          ...
          <div className="mt-4 space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className="shrink-0 font-mono text-[12px] font-bold tabular-nums text-[var(--color-ink)]">$387M</span>
            </div>
            <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--color-ink-muted)]">voter-approved, Nov. 2020 · {HEADLINE.bondApprovalPct}% yes</p>
            <div className="flex items-center gap-3 pt-1">
              <div className="h-7 rounded-sm bg-[var(--color-canopy)]" style={{ width: `${(HEADLINE.bondAuthorizedM / HEADLINE.bondTotalHighM) * 100}%` }} />
              <div className="h-7 rounded-sm bg-[var(--color-ember)]" style={{ width: `${(bondExtra / HEADLINE.bondTotalHighM) * 100}%` }} />
              <span className="shrink-0 font-mono text-[12px] font-bold tabular-nums text-[var(--color-ink)]">~$459M</span>
            </div>
